...
public class UpdateSearchFlightResultView extends JFrame {
	// 窗口中的组件当做类的属性
	...
	JButton ticketOrderBt = new JButton("改签");
	...
	public void addListener() {
		...
		flightMsg.addCaretListener(new CaretListener() {
			...
			public void caretUpdate(CaretEvent e) {
				...
				try {
					...
					takeoffTime = Timestamp.valueOf(flightArr[3].getText().trim());
		...
		ticketOrderBt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				new UpdateAddOrderView("购票人信息", userName, flightNumber, takeoffTime,txArr);
			}
		});
	}
	...
	public void initFrame() {
		...
		try {
			if (takeoffTime != null) {
				List<Flight> flights = flightManager.findFlightByTakeoffTime(takeoffTime);
				for (Flight flight : flights) {
					flightMsg.append(flight + "\n");
				}
			}
			if (flightNumber != null) {
				List<Flight> flightByNum = flightManager.findFlightByFlightNumber(flightNumber);
				for (Flight flight : flightByNum) {
					flightMsg.append(flight + "\n");
				}
			}
			if (startPlace != null) {
				List<Flight> queryFlightByStart = flightManager.fingdFlightByStart(startPlace);
				for (Flight flight : queryFlightByStart) {
					flightMsg.append(flight + "\n");
				}
			}
	...
	public UpdateSearchFlightResultView(String title, String userName, String flightNumber, String startPlace, String endPlace, Timestamp takeoffTime, JTextField[] txArr) {
		super(title);
